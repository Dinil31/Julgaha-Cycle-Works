...
export function handleMonthChange() {
    // Left for backwards compatibility if needed, but updateDashboard handles everything now
    updateDashboard();
}
...
// --- NEW ADVANCED FILTERING DASHBOARD LOGIC ---
export function updateDashboard() {
  ...
  let filtered = rawData.filter((r) => {
    ...
    // Apply Year and Month dropdown filters
    if (y !== "all" && r._date.getFullYear() !== parseInt(y)) return false;
    if (m !== "all" && r._date.getMonth() !== parseInt(m)) return false;

    return true;
  });

  // Determine if we should show a Monthly trend or a Daily trend on the chart
  const isYearView = (m === "all" && dStr === "");

  if (activeTable === "service_logs") {
    renderServiceDashboard(filtered);
  } else {
    renderFinancialDashboard(filtered, isYearView);
  }
}
...
function renderFinancialDashboard(data, isYearView) {
  ...
  data.forEach((r) => {
    ...
    let k;
    if (!isYearView) {
        k = r._date.getDate(); // Show days (1-31) on X-axis
    } else {
      const y = r._date.getFullYear();
      const m = String(r._date.getMonth() + 1).padStart(2, "0");
      k = `${y}-${m}`; // Show Year-Month on X-axis
    }
